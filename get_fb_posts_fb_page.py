...
def get_args():
    parser = \
        argparse.ArgumentParser(description='''Python Script to scrape Facebook Groups and pages.
This will help you scrape the maximum possible data from the page/group.
Things like comments,reacts etc can be scraped and stored to a .csv file.
For more info: https://github.com/minimaxir/facebook-page-post-scraper''')
...
def getReactionsForStatuses(base_url):
    ...
    for reaction_type in reaction_types:
        fields = \
            '&fields=reactions.type({}).limit(0).summary(total_count)'.format(reaction_type.upper())
...
def processFacebookPageFeedStatus(status):
    ...
    status_message = ('' if 'message'
                      not in status else unicode_decode(status['message'
                      ]))
    link_name = ('' if 'name'
                 not in status else unicode_decode(status['name']))
    status_link = ('' if 'link'
                   not in status else unicode_decode(status['link']))

    # Time needs special care since a) it's in UTC and
    # b) it's not easy to use in statistical programs.

    status_published = datetime.datetime.strptime(status['created_time'
            ], '%Y-%m-%dT%H:%M:%S+0000')
    ...
    num_reactions = (0 if 'reactions'
                     not in status else status['reactions']['summary'
                     ]['total_count'])
    num_comments = (0 if 'comments' not in status else status['comments'
                    ]['summary']['total_count'])
    num_shares = (0 if 'shares' not in status else status['shares'
                  ]['count'])
...
def scrapeFacebookPageFeedStatus(
    ...
    with open('{}_facebook_statuses.csv'.format(page_id), 'w') as file:
        ...
        parameters = '/?limit={}&access_token={}'.format(100,
                access_token)
        since = ('&since={}'.format(since_date) if since_date is not ''
                  else '')
        until = ('&until={}'.format(until_date) if until_date is not ''
                  else '')
        ...
        while has_next_page:
            ...
            for status in statuses['data']:
                ...
                num_processed += 1
                if num_processed % 100 == 0:
                    print '{} Statuses Processed: {}'.format(num_processed,
                            datetime.datetime.now())
