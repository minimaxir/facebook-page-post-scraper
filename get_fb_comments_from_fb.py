...
def processFacebookComment(comment, status_id, parent_id = ''):
	...
	if 'attachment' in comment:
		...
		comment_message = attach_tag if comment_message is '' else (comment_message.decode("utf-8") + " " + attach_tag).encode("utf-8")
...
def scrapeFacebookPageFeedComments(page_id, access_token):
	with open('%s_facebook_comments.csv' % file_id, 'wb') as file:
		w = csv.writer(file)
		w.writerow(["comment_id", "status_id", "parent_id", "comment_message", "comment_author", "comment_published", "comment_likes"])
						...
						if 'comments' in comment:
							...
							while has_next_subpage:
								for subcomment in subcomments['data']:
									#print (processFacebookComment(subcomment, status['status_id'], comment['id']))
