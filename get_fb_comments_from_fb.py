...
For more info: https://github.com/minimaxir/facebook-page-post-scraper''')
    ...
    parser.add_argument('-file', type=str,
                        help='ID of the facebook post.', required=True)
...
def processFacebookComment(comment, status_id, parent_id=''):
    ...
    comment_message = ('' if 'message' not in comment
                       or comment['message'] is ''
                        else unicode_decode(comment['message']))
    comment_author = unicode_decode(comment['from']['name'])
    num_reactions = (0 if 'reactions'
                     not in comment else comment['reactions']['summary'
                     ]['total_count'])

    if 'attachment' in comment:
        attachment_type = comment['attachment']['type']
        attachment_type = ('gif' if attachment_type
                           == 'animated_image_share'
                            else attachment_type)
        attach_tag = '[[{}]]'.format(attachment_type.upper())
        comment_message = (attach_tag if comment_message is ''
                            else comment_message + ' ' + attach_tag)

    # Time needs special care since a) it's in UTC and
    # b) it's not easy to use in statistical programs.

    comment_published = \
        datetime.datetime.strptime(comment['created_time'],
                                   '%Y-%m-%dT%H:%M:%S+0000')
...
def scrapeFacebookPageFeedComments(page_id, access_token):
    with open('{}_facebook_comments.csv'.format(file_id), 'w') as file:
        ...
        with open('{}_facebook_statuses.csv'.format(file_id), 'r') as \
            csvfile:
            ...
            for status in reader:
                ...
                while has_next_page:
                    ...
                    for comment in comments['data']:
                        comment_data = processFacebookComment(comment,
                                status['status_id'])
                        reactions_data = reactions[comment_data[0]]

                        # calculate thankful/pride through algebra

                        num_special = comment_data[6] \
                            - sum(reactions_data)
                        ...
                        if 'comments' in comment:
                            ...
                            while has_next_subpage:
                                ...
                                sub_after = ('' if sub_after is ''
                                         else '&after={}'.format(sub_after))
                                sub_base_url = base + sub_node \
                                    + parameters + sub_after

                                sub_url = \
                                    getFacebookCommentFeedUrl(sub_base_url)
                                sub_comments = \
                                    json.loads(request_until_succeed(sub_url))
                                sub_reactions = \
                                    getReactionsForComments(sub_base_url)

                                for sub_comment in sub_comments['data']:
                                    sub_comment_data = \
    processFacebookComment(sub_comment, status['status_id'],
                           comment['id'])
                                    sub_reactions_data = \
    sub_reactions[sub_comment_data[0]]

                                    num_sub_special = \
    sub_comment_data[6] - sum(sub_reactions_data)
        ...
        datetime.datetime.now())
                                ...
                                if 'paging' in sub_comments:
                                    if 'next' in sub_comments['paging']:
                                        sub_after = \
    sub_comments['paging']['cursors']['after']
                                    else:
